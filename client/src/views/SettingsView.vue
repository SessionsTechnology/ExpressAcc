    ...
    <v-window v-model="tab">
      <v-window-item value="general">
        <v-card class="glass-card panel-card settings-panel pa-3 pa-sm-6"><v-card-text><h2 class="section-heading mb-5">Application</h2><v-text-field v-model="settings.applicationName" :disabled="saving" label="Application name" />
          ...
          <v-select v-model="settings.timeZone" :disabled="saving" :items="timeZones" label="Time zone" /><v-text-field v-if="!demoMode" v-model="settings.password" :disabled="saving" label="New admin password" type="password" autocomplete="new-password" hint="Leave blank to keep the current password" persistent-hint class="mb-7" /><v-alert v-else color="primary" variant="tonal" icon="mdi-lock-check-outline" class="mb-7"><strong>Demo credentials are locked.</strong> The admin password, Family Space password, and user PINs cannot be changed.</v-alert>
          <div class="family-access-setting mb-7">
            ...
            <v-alert v-if="demoMode" color="primary" variant="tonal" icon="mdi-lock-check-outline" class="my-5">Family Space remains public in the demo so every visitor can explore it.</v-alert>
            <v-alert v-else-if="settings.familySpaceProtected" color="primary" variant="tonal" icon="mdi-cellphone-check" class="my-5">Signed-in devices stay unlocked for 30 days. Changing or removing this password signs them out immediately.</v-alert>
            <v-alert v-else type="warning" variant="tonal" class="my-5"><strong>Recommended for internet access:</strong> set a Family Space password before exposing Routioneer publicly. It is optional when the app is only reachable on a trusted private network behind your firewall.</v-alert>
            <template v-if="!demoMode"><v-text-field v-model="settings.familyPassword" :disabled="saving || settings.clearFamilyPassword" :label="settings.familySpaceProtected ? 'New Family Space password' : 'Family Space password'" type="password" autocomplete="new-password" :hint="settings.familySpaceProtected ? 'Leave blank to keep the current Family Space password' : 'Use at least 8 characters to protect Family Space'" persistent-hint class="mt-5" />
            <v-checkbox v-if="settings.familySpaceProtected" v-model="settings.clearFamilyPassword" :disabled="saving" color="warning" label="Remove Family Space protection and make it public" hide-details /></template>
          </div>
            ...
            <div class="settings-user-grid">
              ...
              <div class="settings-entry-controls">
                ...
                <div class="settings-entry-actions">
                  <v-btn v-if="user.hasPin && !demoMode" size="small" variant="text" :loading="entryOperation(user) === 'pin'" :disabled="isEntryBusy(user)" @click="clearUserPin(user)">Remove PIN</v-btn>
      ...
      <v-window-item value="data">
        <v-row class="data-card-grid"><v-col cols="12" md="6"><v-card class="glass-card panel-card data-card data-card--backup pa-5 h-100"><v-card-text class="data-card__content"><v-avatar color="success" variant="tonal" class="data-card__icon mb-5"><v-icon icon="mdi-download" /></v-avatar><h2 class="section-heading mb-2">Download backup</h2><p class="muted mb-6">Export settings, users, items, checkouts, chores, and activity as one JSON file.</p><v-spacer /><v-btn class="data-card__action" color="success" href="/api/admin/export" prepend-icon="mdi-download">Download backup</v-btn></v-card-text></v-card></v-col>
          <v-col cols="12" md="6"><v-card class="glass-card panel-card data-card data-card--restore pa-5 h-100"><v-card-text class="data-card__content"><v-avatar color="warning" variant="tonal" class="data-card__icon mb-5"><v-icon :icon="demoMode ? 'mdi-lock-outline' : 'mdi-upload'" /></v-avatar><h2 class="section-heading mb-2">Restore backup</h2><p class="muted mb-6">{{ demoMode ? 'Backup restore is disabled so demo credentials cannot be replaced.' : 'This replaces current application data. The current admin password remains valid.' }}</p><v-file-input v-model="backupFile" :disabled="demoMode || saving || hasEntryOperations" label="Routioneer JSON backup" accept="application/json,.json" prepend-icon="" /><v-spacer /><v-btn class="data-card__action" color="warning" :disabled="demoMode || !backupFile || hasEntryOperations || savingDarkMode" :loading="saving" prepend-icon="mdi-upload" @click="restore">Restore backup</v-btn></v-card-text></v-card></v-col></v-row>
      </v-window-item>
    </v-window>
...
const router = useRouter(); const ready = ref(false); const saving = ref(false); const savingDarkMode = ref(false); const demoMode = ref(false); const tab = ref('general'); const backupFile = ref(null); const openItem = ref(null); const openChore = ref(null)
